...
import (
	...
	"github.com/qj0r9j0vc2/rampart/internal/bootstrap/report"
	...
	"github.com/qj0r9j0vc2/rampart/internal/scan"
)
...
func runGoldenFixture(t *testing.T, name string) {
	...
	configBytes, err := os.ReadFile(filepath.Join(dir, "rampart.yaml"))
	if err != nil {
		t.Fatalf("reading rampart.yaml: %v", err)
	}

	cfg, err := config.Load(filepath.Join(dir, "rampart.yaml"))
	if err != nil {
