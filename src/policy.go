...
func NewAWSPolicy(actions []string, resources bool) (Policy, error) {
	if len(actions) == 0 {
		return Policy{}, fmt.Errorf("actions list cannot be empty")
	}
...
func AWSPolicy(permissions []string, resources bool) (AwsOutput, error) {
	...
	if err != nil {
		...
		return OutPolicy, fmt.Errorf("failed to marshal policy: %w", err)
	}
	...
	if err != nil {
		return OutPolicy, fmt.Errorf("failed to parse template: %w", err)
	}
